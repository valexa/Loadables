...
+(BOOL)punchHoleInSandboxForPath:(NSString*)path denyNotice:(NSString*)denyNotice
{
    ...
    openDlg.prompt = @"Permit access";
    openDlg.title = message;
	[openDlg setAllowsMultipleSelection:NO];    
    [openDlg setShowsHiddenFiles:YES];
    openDlg.directoryURL = [NSURL fileURLWithPath:path];
    openDlg.nameFieldStringValue = path.lastPathComponent;
    BOOL directory;
	...
	if (directory) {
        ...
        openDlg.allowedFileTypes = @[path.pathExtension];        
    }
...
+(NSURL*)getSecurityBookmark:(NSURL*)url
{
    ...
    NSData *bookmark = sandboxedBookmarks[url.URLByDeletingLastPathComponent.path];     //first see if we have a bookmark for the parent directory
    if (bookmark == nil) bookmark = sandboxedBookmarks[url.path];                         //if not then look for a bookmark of the exact file
    if (bookmark) {
        NSError *error = nil;
...
+(BOOL)addSecurityBookmark:(NSURL*)url
{
    ...
    if (error || (bookmarkData == nil)) {
        ...
        NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults]; 
        NSMutableDictionary *sandboxedBookmarks = [NSMutableDictionary dictionaryWithDictionary:[defaults objectForKey:@"sandboxSecureBookmarks"]];        
        sandboxedBookmarks[url.path] = bookmarkData;
        [defaults setObject:sandboxedBookmarks forKey:@"sandboxSecureBookmarks"];
        [defaults synchronize];
    }  
    return YES;
